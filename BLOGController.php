...
/**
 * HTML から PDF を生成するコントローラ。
 *
 * 元はメソッド 3 つがクラス外・PHP 開始タグなしで置かれており、
 * PHP としては読み込めない状態だった。
 */
        ...
        try {
            ...
            $pdf = PDF::loadView('pdf.template', [
              ...
              ->setOptions([
                  ...
                  // isPhpEnabled は必ず false のままにする。
                  // true にすると DomPDF がテンプレート中の
                  // <script type="text/php"> を実行するため、
                  // ユーザーが送ってきた $content からサーバ上で
                  // 任意の PHP を走らせられる（リモートコード実行）。
                  'isPhpEnabled' => false,
                  // 同じ理由でリモートリソースの取得も無効化する（SSRF 対策）。
